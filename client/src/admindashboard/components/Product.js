...
function Product(props) {
  ...
  /*useEffect(() => {
    ...
    //addtocart()
  }, [Admin.box, User.stocks])*/
  
 
  const removefromcart = (inventoryid) => {
    Admin.setBox([...Admin.box.filter((bo) => bo.inventoryid !== inventoryid)])
    User.setStocks([...User.stocks.filter((ar) => !Admin.box.find((rm) => rm.inventoryid === ar.inventoryid))]);
  ...
  if (props.itemtype === "Mouse") cusimage = Mouse;
  if (props.itemtype === "Keyboard") cusimage = Keyboard;
  if (props.itemtype === "CPU") cusimage = CPUs;
  if (props.itemtype === "Monitors") cusimage = Monitor;
  if (props.itemtype === "Printers") cusimage = Printer;
  if (props.itemtype === "UPS") cusimage = UPSs;
  if (props.itemtype === "Laptop") cusimage = Laptop;
  if (props.itemtype === "Tab") cusimage = Tab;
  if (props.itemtype === "Scanner") cusimage = Scanner;
  if (props.itemtype === "Servers") cusimage = Servers;
    ...
    <>
      <div style={{ float: "left", padding: "10px" }}>
        <div className='card-image'>
          {props.in === "sidebar" ? null : props.image ? (
            <img
        ...
        >
          {`${props.srno}/In cart:${props.inCart}`}
        </i>
        <div className='card-heading'>{props.brandname}</div>
        <div className='card-text'>{props.itemtype}</div>
        <div className='card-text'>{props.itemname}</div>
        <div className='card-text'>{props.serialno}</div>
        <div className='card-text'>
          Warranty Ends:
          {props.warranty_end_date ? (
            GetCurrentDate() > props.warranty_end_date ? (
              <i className='alert-danger'>
                {dateFormat(props.warranty_end_date, "mmmm dS yyyy")}
              </i>
            ) : (
          ...
          props.in === "sidebar" ? (
            ...
            <>
              ...
              <button
                ...
                onClick={() => props.addcart(props.stock)}
              >
                {" "}
                Add To Set
              </button>
      ...
      {showDescriptionTab ? (
        <Description
          ...
          warranty_end_date={props.warranty_ends_on}
